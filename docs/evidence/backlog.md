...
|---|---|---|---|---|
| AS-01a | **unverified** | `residue` does not enumerate the git lease mode's refs — but no longer reads as if it did | The enumerating read walks `.agent-sync/leases/*.lock`, which BOTH modes write — `_note_local` leaves a note whenever a ref is won here. A ref won on another machine has no local note, so it is absent from the report. **Half closed 2026-08-20:** in git mode `residue` now prints `⚠ INCOMPLETE IN THIS MODE`, names the authority (`refs/agent-sync/leases/*` on the configured remote) and gives the `git ls-remote` line that enumerates them by hand — fixture `residue states what it cannot see in git mode (AS-01a)`. What is still open is the **sweep itself**, and reaping one needs the same `--force-with-lease` compare-and-swap `_git_release` uses. Deferred on purpose: an unconditional sweep puts a network round-trip in front of `status`, which is the reasoning `_note_local` already carries | AS-01, 2026-08-19 · restated 2026-08-20 |
