...
|---|---|---|---|---|
| **ASY-03** | unverified | **The behavioral evals have never been executed.** `test/evals/` ships scenarios and triggers, `evals_validate.py` checks their SHAPE in CI, and no dated run of the evals themselves exists — `test/evals/RESULTS.md` records no execution. A suite that has only been linted is a suite whose pass rate is unknown. The fix is a dated run with results committed, not more validation of the data files. | Named by the 2026-08-29 family audit (ASY-03) and deliberately left out of the wave-2 release, which was guard/hook hardening; deferred out loud rather than by going quiet. | audit 2026-08-29, deferred 2026-08-30 |
