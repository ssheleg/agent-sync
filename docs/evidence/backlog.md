...
|---|---|---|---|---|
| AS-01a | **unverified** | `residue` does not enumerate the git lease mode's refs — but no longer reads as if it did | The enumerating read walks `.agent-sync/leases/*.lock`, which BOTH modes write — `_note_local` leaves a note whenever a ref is won here. A ref won on another machine has no local note, so it is absent from the report. **Half closed 2026-08-20:** in git mode `residue` now prints `⚠ INCOMPLETE IN THIS MODE`, names the authority (`refs/agent-sync/leases/*` on the configured remote) and gives the `git ls-remote` line that enumerates them by hand — fixture `residue states what it cannot see in git mode (AS-01a)`. What is still open is the **sweep itself**, and reaping one needs the same `--force-with-lease` compare-and-swap `_git_release` uses. Deferred on purpose: an unconditional sweep puts a network round-trip in front of `status`, which is the reasoning `_note_local` already carries | **closed 2026-08-20** — `git_residue()` enumerates `refs/agent-sync/leases/*` on the configured remote and classifies each ref with the same `classify_lock` the local plane uses; `git_reap()` clears a reapable one with the same `--force-with-lease` compare-and-swap `_git_release` uses and proves it went by re-reading `ls-remote`. Four fixtures against a real bare remote, `npm test` exit 0 — 51 self-test fixtures, 19 claim-cell cases (was 16). The fixture that asserted `INCOMPLETE IN THIS MODE` was rewritten rather than kept: it encoded the half-closed state, and a check that CAN look must not keep printing that it cannot |
| AS-01b | **unverified** | The 17 expired locks in the family are reported and still there | This row's mechanism classifies all 17: `foreign` to any fresh session, `ambiguous` to a plain shell, `reapable` only by the session that took them — so no run today can clear one, by design. Whether they are reaped is the orchestrator's decision. **The sub-count was wrong and is now computed:** the row said "14 of the 17", and by repository it is **16 outside this checkout, 13 outside this checkout and the umbrella** — 1 here, 3 in the umbrella, 13 in the other seven members (2026-08-20). Every lock in the family carries `ttl: 2700`, so `-mmin +45` reproduces expiry without reading one:<br>`find ~/DATA/sshlg-skills -path '*/.agent-sync/leases/*.lock' -mmin +45 \| wc -l` → 17<br>`… -not -path '*/skills/agent-sync/*' \| wc -l` → 16<br>`find ~/DATA/sshlg-skills/skills … -not -path '*/agent-sync/*' \| wc -l` → 13 | **closed 2026-08-20** — the row's real subject was that an `ambiguous` lock is unclearable BY ANYONE, for ever. `reap --i-own-this <key>` is the operator's decision: named keys only, refuses a blanket sweep, refuses a live lease, prints the payload it destroyed and journals it, and leaves the classifier's verdict unchanged beside it. Five fixtures (23 cases, was 19). **Applied to the real state: 28 locks across this machine → 0 in the family**, each cleared by name with what it held printed. Two defects in the override surfaced from running it on that state rather than on fixtures: a key beginning with a dash needed `--`, and the family sweep I wrote around it was wrong where the command was right |
