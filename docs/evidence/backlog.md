...
|---|---|---|---|---|
| **ASY-10** | unverified | **q06 is a stable trigger miss: the submodule-finish query routes past the skill on every sample.** "Finish a submodule change so the parent points at the pushed commit" carries no coordination vocabulary, and the SKILL.md description has no submodule / parent-pointer literal to catch it — measured 2026-08-31 at 0/3 on haiku (all `none`) and 0/3 on sonnet (all `task-pipeline`); every other query passed on both models (`test/evals/RESULTS.md`). The fix is a description change (a submodule/parent-pointer trigger literal), which is a minor release and a YAML-fragile edit — out of the v1.18.7 patch on purpose. | The 2026-08-31 eval run (ASY-03 closed) — the only failing cell in 24 trigger probes, and it is a positive: the multi-repository finish protocol is advertised in scenarios (s03) but not reachable from the words a user would actually say. | eval run 2026-08-31 |
